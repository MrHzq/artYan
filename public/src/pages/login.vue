                ...
                <div class="register" v-show="status==='register'">
                    <div class="regFirstStep" v-show="registerStep===1">
                        ...
                        <div class="inputBox" key="6">
                            <div class="tips">请输入手机号</div>
                            <input type="text" @keyup.enter="registerFun('.register')" @focus="teXiao" v-model="register.username">
                            <div class="line greyLine"></div>
                            <div class="line redLine"></div>
                        </div>
                        <!-- 输入密码 -->
...
export default {
    ...
    methods: {
        ...
        errTeXiao(inputBox, index) {
            ...
            inputBox[index].lastChild.style.width = "100%";
        },
        ...
        noErrTeXiao(inputBox) {
            ...
            for (let i = 0; i < inputBox.length; i++) {
                inputBox[i].lastChild.style.width = "0";
            }
        },
        // 判断是否为手机号
        isPhone(value) {
        ...
        loginFun(flag) {
            ...
            if (username && password) {
                let txt = ["用户名不存在，请注册", "密码错误", "您输入的不是手机号"];
                // 1、判断是否为手机号
        ...
        firstStepFun(flag) {
            let { username } = this.forget,
                ...
                txt = ["验证码错误", "用户名不存在，请注册", "您输入的不是手机号", "请输入验证码"];
            //1、判断 账号、验证码 是否为空
            //2、判断输入的验证码是否正确
            //3、验证码正确，查找用户名是否存在
